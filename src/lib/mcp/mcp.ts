...
export async function generateCampaignStrategy(custom?: CustomSegmentRequest): Promise<CampaignStrategyResult> {
  try {
    ...
    if (custom?.filters && custom.filters.length > 0) {
      for (const client of clients) {
        for (const filter of custom.filters) {
          const { field, match, currentMonth, min, max } = filter;
          ...
          if (field === 'birthDate') {
            ...
            if (currentMonth && birth.getMonth() !== new Date().getMonth()) continue;
            if (min && birth < new Date(min)) continue;
            if (max && birth > new Date(max)) continue;
    ...
    throw new Error(`Campaign strategy generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}