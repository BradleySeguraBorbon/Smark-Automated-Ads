...
const SYSTEM_PROMPT = `
  ...
  Never generate the JSON manually.

  Never return a static object like { segmentId: ... } yourself.

  Only respond by calling the segmentAudience tool — the server will execute the tool and return the segmentId.
  
  Always return JSON in the exact format required by the application. Do not explain your reasoning. Never return Markdown or lists. Return only a single valid JSON object generated by the tool call matching this structure:
  { segmentId: string}
`.trim();
